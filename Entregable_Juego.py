...
class TicTacToe:
    ...
    def checkWinner(self):
        if self.is_player_win(self.board,self.humanPLayer):
            os.system("cls")
            print(f"   Jugador {self.humanPLayer} gano!")
            return True
            
        if self.is_player_win(self.board,self.botPlayer):
            os.system("cls")
            print(f"   Jugador {self.botPlayer} gano!")
            return True
    ...
    def start(self):
        ...
        while True:
            os.system("cls")
            print(f"   Jugador {self.humanPLayer} del usuario")
            self.show_board()
...
class humanPLayer:
    ...
    def human_move(self,state):
        ...
        while True:
            square =  int(input("Colocar un numero del 1 al 9 en cada cuadro dependiendo si es X - 0: "))
            print()
            if state[square-1] == "-":
                break
        return square-1
